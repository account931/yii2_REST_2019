...
class BookingCphV2HotelController extends Controller
{
 ...
 // **                                                                                  **
	 ...
	 //$all_records = BookingCphV2Hotel::find()->where(['booked_by_user' => Yii::$app->user->identity->username])->all();
	 
	 
	 $model->booked_by_user = Yii::$app->user->identity->username; //fill the field "Who is booking" default value
    ...
    // **                                                                                  **
	 public function actionAjax_delete_1_booking() //ajax
     {
	    ...
	    $thisMonthData = BookingCphV2Hotel::find() 
			 ...
			 ->where([ 'booked_by_user' => Yii::$app->user->identity->username])
			 -> one() 
			 -> delete();  
